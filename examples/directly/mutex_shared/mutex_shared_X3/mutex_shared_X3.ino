...
#define REQEST_X3_PIN 2    // try_acquired сигнал -> mutex
#define RESPONSE_X3_PIN 4  // ответ mutex -> X3
#define RELEASE_X3_PIN 7   // сигнал release -> mutex
...
void setup() {
    ...
    pinMode(RESPONSE_X3_PIN, INPUT);
    pinMode(IN_LAMP_PIN, OUTPUT);
    pinMode(REQEST_X3_PIN, OUTPUT);
    pinMode(RELEASE_X3_PIN, OUTPUT);
...
void loop() {
    ...
    bool is_acquired = digitalRead(RESPONSE_X3_PIN);


    // Braccio control

    control.go_step(input_X3, is_acquired, true, true, true, true, true, true);


    // Try to acquire

    if (control.want_to_acquire){
        digitalWrite(REQEST_X3_PIN, HIGH);
    } else {
        digitalWrite(REQEST_X3_PIN, LOW);
    }
    ...
    if (control.want_to_release){
        digitalWrite(RELEASE_X3_PIN, HIGH);
    } else {
        digitalWrite(RELEASE_X3_PIN, LOW);
    }

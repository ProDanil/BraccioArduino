#ifndef Controller1_hpp
#define Controller1_hpp

struct ControllerX1 {
    enum struct AngleBase {
        A0 = 0,
        A180 = 180
    };
    enum struct AngleShoulder {
        A90 = 90,
        A115 = 115,
        A130 = 130,
        A140 = 140,
        A160 = 160
    };
    enum struct AngleElbow {
        A0 = 0,
        A180 = 180,
    };
    enum struct AngleWristVer {
        A10 = 10,
        A15 = 15,
        A30 = 30,
        A60 = 60
    };
    enum struct AngleWristRot {
        A0 = 0,
        A90 = 90,
    };
    enum struct AngleGripper {
        A15 = 15,
        A73 = 73,
    };

    enum State {
        BEGIN, 
        GO_WAIT, 
        WAIT, 
        GO_UP, 
        GO_LOW, 
        GO_PICKUP, 
        GO_UP_CLENCHED,
        GO_UP_OUT_CLENCHED,
        WAIT_ACK, 
        GO_DROP, 
        GO_SAFE,
        WAIT_ACK_SAFE,
        __SAME__
    } state = BEGIN;

    struct Input {
        bool want_cargo_on_out;
        bool ack;
        bool is_done_m1;
        bool is_done_m2;
        bool is_done_m3;
        bool is_done_m4;
        bool is_done_m5;
        bool is_done_m6;

        bool operator==(const Input& other) const {
            return want_cargo_on_out == other.want_cargo_on_out &&
                   ack == other.ack &&
                   is_done_m1 == other.is_done_m1 &&
                   is_done_m2 == other.is_done_m2 &&
                   is_done_m3 == other.is_done_m3 &&
                   is_done_m4 == other.is_done_m4 &&
                   is_done_m5 == other.is_done_m5 &&
                   is_done_m6 == other.is_done_m6;
        }
    };

    struct Out {
        /* TODO: initial values */
        AngleBase go_base;
        AngleShoulder go_shoulder;
        AngleElbow go_elbow;
        AngleWristVer go_wrist_ver;
        AngleWristRot go_wrist_rot;
        AngleGripper go_gripper;
        bool ready;
    } out;

    String state2string(State s) {
        switch (s) {
            case BEGIN:
                return "BEGIN";
            case GO_WAIT:
                return "GO_WAIT";
            case WAIT:
                return "WAIT";
            case GO_UP:
                return "GO_UP";
            case GO_LOW:
                return "GO_LOW";
            case GO_PICKUP:
                return "GO_PICKUP";
            case GO_UP_CLENCHED:
                return "GO_UP_CLENCHED";
            case GO_UP_OUT_CLENCHED:
                return "GO_UP_OUT_CLENCHED";
            case WAIT_ACK:
                return "WAIT_ACK";
            case GO_DROP:
                return "GO_DROP";
            case GO_SAFE:
                return "GO_SAFE";
            case WAIT_ACK_SAFE:
                return "WAIT_ACK_SAFE";
            case __SAME__:
                return "__SAME__";
            default:
                return "";
        }
    }

    bool go_step(Input input) {
        bool is_done_all = input.is_done_m1 &&
                           input.is_done_m2 &&
                           input.is_done_m3 &&
                           input.is_done_m4 &&
                           input.is_done_m5 &&
                           input.is_done_m6;

        State next_state = __SAME__;

        // Compute next state
        if (0) {
        } else if (state == BEGIN) {
            next_state = GO_WAIT;

        } else if (state == GO_WAIT && is_done_all) {
            next_state = WAIT;

        } else if (state == WAIT && input.want_cargo_on_out) {
            next_state = GO_UP;

        } else if (state == GO_UP && is_done_all && input.want_cargo_on_out) {
            next_state = GO_LOW;

        } else if (state == GO_LOW && is_done_all && input.want_cargo_on_out) {
            next_state = GO_PICKUP;

        } else if (state == GO_PICKUP && is_done_all && input.want_cargo_on_out) {
            next_state = GO_UP_CLENCHED;

        } else if (state == GO_UP_CLENCHED && is_done_all) {
            next_state = GO_UP_OUT_CLENCHED;

        } else if (state == GO_UP_OUT_CLENCHED && is_done_all) {
            next_state = WAIT_ACK;

        } else if (state == WAIT_ACK && input.ack) {
            next_state = GO_DROP;

        } else if (state == GO_DROP && is_done_all) {
            next_state = GO_SAFE;

        } else if (state == GO_SAFE && is_done_all) {
            next_state = WAIT_ACK_SAFE;

        } else if (((state == WAIT_ACK_SAFE && input.ack) || state == GO_UP || state == GO_LOW || state == GO_PICKUP) && is_done_all) {
            next_state = GO_WAIT;
        }

        // Compute outputs
        if (0) {
        } else if (next_state == GO_WAIT) {
            out.go_base = AngleBase::A0;
            out.go_shoulder = AngleShoulder::A140;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A10;
            out.go_wrist_rot = AngleWristRot::A0;
            out.go_gripper = AngleGripper::A15;
        } else if (next_state == WAIT) {
            out.ready = false;
        } else if (next_state == GO_UP) {
            out.go_base = AngleBase::A180;
            out.go_shoulder = AngleShoulder::A115;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A15;
            out.go_wrist_rot = AngleWristRot::A90;
            out.go_gripper = AngleGripper::A15;
        } else if (next_state == GO_LOW) {
            out.go_base = AngleBase::A180;
            out.go_shoulder = AngleShoulder::A90;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A10;
            out.go_wrist_rot = AngleWristRot::A90;
            out.go_gripper = AngleGripper::A15;
        } else if (next_state == GO_PICKUP) {
            out.go_base = AngleBase::A180;
            out.go_shoulder = AngleShoulder::A90;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A10;
            out.go_wrist_rot = AngleWristRot::A90;
            out.go_gripper = AngleGripper::A73;
        } else if (next_state == GO_UP_CLENCHED) {
            out.go_base = AngleBase::A180;
            out.go_shoulder = AngleShoulder::A115;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A15;
            out.go_wrist_rot = AngleWristRot::A90;
            out.go_gripper = AngleGripper::A73;
        } else if (next_state == GO_UP_OUT_CLENCHED) {
            out.go_base = AngleBase::A0;
            out.go_shoulder = AngleShoulder::A130;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A60;
            out.go_wrist_rot = AngleWristRot::A90;
            out.go_gripper = AngleGripper::A73;
            out.ready = true;
        } else if (next_state == WAIT_ACK) {
            out.ready = false;
        } else if (next_state == GO_DROP) {
            out.go_base = AngleBase::A0;
            out.go_shoulder = AngleShoulder::A130;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A60;
            out.go_wrist_rot = AngleWristRot::A90;
            out.go_gripper = AngleGripper::A15;
        } else if (next_state == GO_SAFE) {
            out.go_base = AngleBase::A0;
            out.go_shoulder = AngleShoulder::A140;
            out.go_elbow = AngleElbow::A0;
            out.go_wrist_ver = AngleWristVer::A10;
            out.go_wrist_rot = AngleWristRot::A0;
            out.go_gripper = AngleGripper::A15;
        } else if (next_state == WAIT_ACK_SAFE) {
            out.ready = true;
        } else {
            // do nothing
        }

        // Update the state
        if (next_state != __SAME__) {
            state = next_state;
        }

        // return is_active
        return (next_state != __SAME__);
    }
};

#endif
